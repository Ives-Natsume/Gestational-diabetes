...
lasso_flora_fun_s1 <- function(dataset, col_1, col_2, target) {
  ...
  ylasso <- normal_dataset[, target]  # 此处y为总胆固醇
...
repeat {
  ...
  if (flag_lasso+10 >= ncol(flora_base))  {
    print(lasso_flora_fun_s1(base,flag_lasso,ncol(flora_base)+1,flag_target))
    flag_lasso <- 1
    break
  }
}
